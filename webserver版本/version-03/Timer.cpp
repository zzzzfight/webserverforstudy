...
// class Channel;
using namespace std;
TimerNode::TimerNode(std::shared_ptr<HttpData> &ptr, size_t timeout)
...
void Timer::add_timer(shared_ptr<HttpData> ptr, size_t timeout)
{
	shared_ptr<TimerNode> newnode(new TimerNode(ptr, timeout));
	// _queue.
	cout << "push new node\n";
	_queue.push(newnode);
	ptr->LinkTimer(newnode);
}
...
void Timer::HandleExpiredEvent()
{
	while (!_queue.empty())
	{
		...
		{
			cout << "pop a node\n";
			cout << "Timer.cpp:node->GetEspiredTime:" << node->GetExpiredTime() << endl;
			_queue.pop();
		}
