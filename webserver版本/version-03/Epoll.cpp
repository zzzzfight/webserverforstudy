...
void Epoll::Epoll_add(std::shared_ptr<Channel> chn, int timeout)
{
	...
	if (timeout > 0)
	{
		...
		{
			_fd2http[curfd] = http;
			_timemanager.add_timer(http, timeout);
		}
		else
		{
			abort();
...
void Epoll::Epoll_mod(std::shared_ptr<Channel> chn, int timeout)
{
	if (timeout > 0)
	{
		auto _http = chn->GetHolder();
		if (_http)
		{
			_timemanager.add_timer(_http, timeout);
		}
		else
		{
			abort();
...
std::vector<std::shared_ptr<Channel>> Epoll::Polling()
{
	...
	int eventnum = epoll_wait(_epollfd, _events, MAXEVENTS, 10000);
