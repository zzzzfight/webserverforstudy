...
int main()
{
	int fd = open("./http.txt", O_RDONLY);
	char buf[1024];
	int rn = read(fd, buf, 1024);
	HttpData http;
	// string buffer(buf);
	http.inbuffer.clear();
	http.inbuffer.append(buf);
	http.RequestLineAna();
	cout << endl;
	// int enterpos = buffer.find_first_of('\n');
	// if (enterpos)
	// {
	// 	string headline = buffer.substr(0, enterpos);
	// 	buffer = buffer.substr(enterpos + 2);
