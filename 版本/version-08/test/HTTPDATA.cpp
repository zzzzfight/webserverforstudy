...
int main()
{
	...
	while ((connfd = accept(sockfd, (sockaddr *)&connaddr, &connaddr_len)) > 0)
	{
		...
		// // int readnum = readn(connfd, http.inbuffer);
		char buf[1024];
		memset(buf, '\0', 1024);
		int readnum = read(connfd, buf, 1024);
		cout << readnum;
		// http._inbuffer += std::string(buf, buf + readnum);
		http._inbuffer.clear();
		http._inbuffer.append(string(buf, buf + 1024));
		// http._inbuffer += std::string(buf, buf + readnum);
		cout << http._inbuffer << endl;
		int pos = http._inbuffer.find_first_of("/n/r");
		if (pos)
		{
			string headline = http._inbuffer.substr(0, pos);
			// http._inbuffer.
			// http._mothed = http._inbuffer.find("GET");
		...
		// cout << http._inbuffer. << endl;
		http._inbuffer.clear();
		// read(connfd, http._inbuffer, 1024);
		close(connfd);
	}
	mesg = strerror(errno);
	perror(mesg);
